...
if __name__ == '__main__':
	...
	for i in range(1, 26):
		if not os.path.exists(f'{args.root}/day{i}'):
			os.mkdir(f'{args.root}/day{i}')
			days_created[i-1] = True

		if days_created[i-1] or args.f:
			# Create an empty file for the input
			with open(f'{args.root}/day{i}/input{i}.txt', 'w'):
				pass

			# Create a python script with boilerplate code for the solution
			with open(f'{args.root}/day{i}/solve{i}.py', 'w') as f:
				f.write('import os\n\n')
				f.write(f"input_file = os.path.join(os.path.dirname(__file__), 'input{i}.txt')\n")
